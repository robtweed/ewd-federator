...
var ewdChild = {
    ...
    (function(request, args) {
      client.run(args, function(error, data) {
        ...
        if (error) {
          ...
          if (ewdChild.service && ewdChild.service[request.type] && ewdChild.service[request.type].GraphQLSchema) {
            responseObj.contentType = 'text/json; charset=utf-8';
            responseObj.response = JSON.stringify(data);
          }
  ...
  sendToSite: function(params) {
    ...
    //console.log('sendToSite: params = ' + JSON.stringify(params));
  ...
  messageHandlers: {
    ...
    restRequest: function(messageObj) {
      ...
      //console.log('destinationObj: ' + JSON.stringify(destinationObj));
      ...
      for (name in rest.params) {
        if (name !== '0' && name !== '1' && !rest.query[name]) {
          if (typeof rest.params[name] === 'object') {
            post_data[name] = JSON.stringify(rest.params[name]);
          }
          else {
      ...
      if (serviceObj && serviceObj.GraphQLSchema) {
        post_data.GraphQLSchema = serviceObj.GraphQLSchema;
      }
      type = rest.params[1];
